...
function musicTone(freq,duration=.32,type='triangle',gain=.018){
 if(!musicEnabled||!freq)return;try{audio??=new (window.AudioContext||window.webkitAudioContext)();if(audio.state==='suspended')audio.resume();const o=audio.createOscillator(),g=audio.createGain();o.type=type;o.frequency.value=freq;g.gain.setValueAtTime(.0001,audio.currentTime);g.gain.exponentialRampToValueAtTime(gain,audio.currentTime+.035);g.gain.exponentialRampToValueAtTime(.0001,audio.currentTime+duration);o.connect(g).connect(audio.destination);o.start();o.stop(audio.currentTime+duration+.03)}catch{}
}
...
function feed(){if(!started||paused||win||life<=0||throwCooldown>0)return;if(food<=0){notice('🍎 Еда закончилась! Яблоки растут на деревьях — подпрыгни, чтобы сорвать. Грибы и капуста растут на земле.');sound(170,.2);return}if(mountedFriend&&friend&&friendHP<diffCfg().friendHP){food--;throwCooldown=.6;friendHP=Math.min(diffCfg().friendHP,friendHP+1);statsData.fed++;sound(520,.16,'sine');notice(`💚 Друг подкрепился: 🐗❤️ ${friendHP}/${diffCfg().friendHP}`);hud();return}food--;throwCooldown=.6;const start=boy.position.clone().add(new THREE.Vector3(0,1,0));const dir=new THREE.Vector3(Math.sin(yaw),0,Math.cos(yaw));const target=start.clone().addScaledVector(dir,10);const foeNear=foes.filter(f=>f.g.position.distanceTo(target)<4.5||f.g.position.distanceTo(boy.position)<9).sort((a,b)=>a.g.position.distanceTo(target)-b.g.position.distanceTo(target))[0];const friendNear=friend&&friendHP<diffCfg().friendHP&&friend.g.position.distanceTo(boy.position)<6.5?friend:null;let near=foeNear;if(friendNear){const fd=friendNear.g.position.distanceTo(target),ed=foeNear?foeNear.g.position.distanceTo(target):Infinity;if(fd<3.2&&fd<=ed+.6)near=friendNear}if(near)target.copy(near.g.position).add(new THREE.Vector3(0,.8,0));const types=['apple','mushroom','cabbage'],type=types[Math.floor(Math.random()*types.length)],foodObj=makeFoodModel(type,scene,start.x,start.y,start.z);shots.push({g:foodObj,start,target,t:0,foe:near,type});boy.children[5].rotation.x=-1.1;sound(650,.12,'triangle');notice(near?`Тимур бросил ${foodLabel(type)}!`:`${foodLabel(type)} летит вперёд — целься в кабана!`);hud()}
...
function updateFamilyHouseReveal(){
 ...
 // Trigger already in the doorway, then keep the whole father's house translucent while Timur is inside.
 const nowInside=local.x>-2.25&&local.x<.95&&local.z>-1.85&&local.z<2.55;
...
function repairAndAuditSpawns(){
 ...
 for(const f of familyMembers){if(level===3&&f.role===1)continue;if(spawnPointBlocked(f.g.position.x,f.g.position.z,.55)){const q=nearestSafeSpawn(f.g.position.x,f.g.position.z,.55);if(q){f.g.position.x=q[0];f.g.position.z=q[1];f.x=q[0];f.z=q[1];repairs.push(`family${f.role}`)}else issues.push(`spawn-blocked:family${f.role}`)}}
 ...
 familyMembers.forEach((f,i)=>{if(!(level===3&&f.role===1))check(f.g.position.x,f.g.position.z,.55,`family${i}`)});
...
if(!mountedFriend&&jump&&friend?.g&&!friend.flee&&py<.35&&Math.hypot(boy.position.x-friend.g.position.x,boy.position.z-friend.g.position.z)<2.65){jump=false;mountFriendNow()}
const moveSpeed=mountedFriend?8.6:6;let mx=dx*dt*moveSpeed,mz=dz*dt*moveSpeed;
