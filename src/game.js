...
if(mountedFriend){
 ...
 if(friend?.g&&!friend.flee&&py>.62&&py<2.35&&vy<=2.2&&Math.hypot(boy.position.x-friend.g.position.x,boy.position.z-friend.g.position.z)<1.72){mountedFriend=true;py=1.48;vy=0;boy.position.x=friend.g.position.x;boy.position.z=friend.g.position.z;boy.position.y=py;notice('🐗 Верхом! Прыжок — спрыгнуть. Кабанчик бежит быстрее!')}
}
