...
boy.userData.visualRemaster=true;boy.traverse(o=>{if(o.isMesh){o.castShadow=!mobile;o.receiveShadow=!mobile}});scene.add(boy);boy.position.set(0,0,4);let vy=0,py=0,mountedFriend=false;const DIFF_NAMES=['Я слишком мал, чтобы умереть','Не мучай меня, кабанчик','Ультра-кабан','Кошмар','НЕВОЗМОЖНО'];
...
$('saveResult').onclick=async()=>{const name=($('playerName').value.trim()||'Аноним').slice(0,20);$('saveResult').disabled=true;$('saveMsg').textContent='💾 Сохраняю результат…';let localOk=true;try{localStorage.setItem('kabanchiki3d_player_name',name);const a=JSON.parse(localStorage.getItem('kabanchiki3d_results')||'[]');a.unshift({name,score,time:Math.round(totalTime),win,level,difficulty:selectedDiff,family:familyFound,version:GAME_VERSION,date:new Date().toLocaleDateString()});localStorage.setItem('kabanchiki3d_results',JSON.stringify(a.slice(0,50)))}catch{localOk=false}const global=await submitGlobalResult(name);if(win&&global.ok)$('saveMsg').textContent='✅ Результат сохранён на устройстве и в 🌍 мировом рейтинге!';else if(win)$('saveMsg').textContent=(localOk?'✅ Локально сохранено. ':'⚠️ Локальное сохранение недоступно. ')+'🌍 Мировой рейтинг сейчас недоступен.';else $('saveMsg').textContent=localOk?'✅ Результат сохранён на этом устройстве. В мировой рейтинг попадают победы.':'⚠️ Браузер запретил локальное сохранение.';$('saveResult').disabled=false};function restartGame(){
...
if(mountedFriend){
 ...
 else{py=1.48;vy=0;friend.g.position.x=boy.position.x;friend.g.position.z=boy.position.z;friend.g.rotation.y=boy.rotation.y;boy.position.y=py}
}else{
 if(jump&&py<=0.001){vy=7;jump=false}
 vy-=18*dt;py=Math.max(0,py+vy*dt);if(py===0)vy=0;boy.position.y=py;
 if(friend?.g&&!friend.flee&&py>.72&&vy<=1.5&&Math.hypot(boy.position.x-friend.g.position.x,boy.position.z-friend.g.position.z)<1.48){mountedFriend=true;py=1.48;vy=0;boy.position.x=friend.g.position.x;boy.position.z=friend.g.position.z;boy.position.y=py;notice('🐗 Верхом! Прыжок — спрыгнуть. Кабанчик бежит быстрее!')}
}
